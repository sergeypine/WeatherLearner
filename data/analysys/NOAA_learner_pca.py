...
def exploreModel(featuresetFile, n):
	...
	#  normalize data
	scaler = MinMaxScaler() 
	X = scaler.fit_transform(featureset.drop(columns = ['Y', 'prediction_for_ts']))
	...
	#(3) Train/Test Split
	X_train, X_test = piecewiseSplit(feature_reduced_X, 120, 30, 5)
	print("Piecewise Split: trainset len = {}, testset_len = {}".format(len(X_train), len(X_test)))
	Y_train, Y_test = piecewiseSplit(featureset[['Y']], 120, 30, 5)
...
files = ['NOAA_featureset__is_precip_LocationCount9_LookBack3h_LookAhead12h.csv', 'NOAA_featureset__is_precip_LocationCount9_LookBack3h_LookAhead24h.csv', 
	...
	'NOAA_featureset_Temp_LocationCount9_LookBack3h_LookAhead12h.csv']

for file in files:
		exploreModel(file, 40)
		print("=================\r\n")
