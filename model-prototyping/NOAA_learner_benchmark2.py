...
def exploreModel(mission):
	...
	# Split the data in 50-day chunks; of each day, 35 are for training, 12 are for testing and 3 are skipped to prevent data leakage
	train_df, val_df = piecewiseSplit(featureset, 35, 12, 3)
	...
	if "is_" in mission.predictedVariable:
		esCallback = tf.keras.callbacks.EarlyStopping(monitor='val_recall', mode="max", patience=10, min_delta = 0.0025, restore_best_weights = True)
	else:
		esCallback = tf.keras.callbacks.EarlyStopping(monitor='val_root_mean_squared_error', mode="min", patience=10, min_delta = 0.0025, restore_best_weights = True)
